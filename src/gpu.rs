...
impl Uniforms {
    pub fn new(time: f32, resolution: [f32; 2], depth: u32) -> Self {
        Self {
            time,
            _pad0: 0.0,
            resolution,
...
/// Owns every GPU resource: the surface, device/queue, and the single
/// full-screen-triangle pipeline that draws the fractal.
...
pub async fn init_wgpu(window: Arc<Window>) -> RenderState {
